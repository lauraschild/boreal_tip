...
As the analyses and associated download are quite computation (and time) intensive we provide three options:
	1. Running a geographical subset.
		...
		when running the main script for the subset analysis. The Zenodo directory has a size of 12GB (downloading depends on your network speed but takes approximately xxx).
	...
	2. Running the entire analyses
		...
		this analysis option. The Zenodo directory has a size of 12GB (downloading depends on your network speed but takes approximately xxx). It includes the MODIS data, Pollen
		data and paths for the other data sources.
		The analysis will take a long time due to data extraction and surrogate creation. Once the downloads are complete it will take approximately xxx.
...
Data availability:
	- data download is automated using download paths and wget scripts. Below is a list of stable doi, should the automatic downloading cease to work.
	- MPI
