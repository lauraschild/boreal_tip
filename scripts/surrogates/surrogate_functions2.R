...
tip_it_paleoREVEALS <- function(temperature_vector,
  ...
  get_biome <- function(temperature){
    ...
    intervals <- c(17,25,100)
    possible_biomes <- list(c("boreal_forest"),
  ...
  #biome information
  biomes <- c("boreal_forest","steppe","gaussian")
  cover <- c(75,7,124)
  ...
  for(new_temperature in temperature_vector[-1]){
    ...
    if(old_biome == "boreal_forest"){
      new_biome <- old_biome
      if(new_temperature >= 25| new_temperature <= 15){
  ...
  c <- 5  #SD
  model_cover[which(model_biomes == "gaussian")] <- a*exp(-(((temperature_vector[which(model_biomes=="gaussian")]-b)^2)/(2*c^2)))
