...
tip_it_paleoREVEALS <- function(temperature_vector,
  ...
  #biome information
  biomes <- c("tundra","savanna1","savanna2","savanna3","boreal_forest","steppe")
  cover <- c(5,60,25,70,85,12.5)
  ...
  for(new_temperature in temperature_vector[-1]){
    ...
    if(old_biome == "boreal_forest"){
      new_biome <- old_biome
      if(new_temperature >= 22.5| new_temperature <=12){
    ...
    if(old_biome == "savanna1"){
      new_biome <- old_biome
      if(new_temperature <= 7.5 | new_temperature >= 13){
        new_biome <- get_biome(new_temperature)
      }
    }
    ...
    if(old_biome == "savanna2"){
      new_biome <- old_biome
      if(new_temperature >= 17.5 | new_temperature <=10){
        new_biome <- get_biome(new_temperature)
      }
    }
    if(old_biome == "savanna3"){
      new_biome <- old_biome
